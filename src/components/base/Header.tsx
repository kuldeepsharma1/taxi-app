...
export default function Header() {
    ...
    const handleNavLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, item: string) => {
        e.preventDefault();
        const headerHeight = headerRef.current?.offsetHeight || 70; // Adjust based on actual scrolled header height
    ...
    return (
        ...
        >
            <div className="container mx-auto flex justify-between items-center px-4 sm:px-6 lg:px-8">
                ...
                >
                    Himachal<span className={isScrolled || isMobileMenuOpen ? 'text-slate-800' : 'text-sky-300'}>Ride</span>
                </a>
                <nav className="hidden lg:flex space-x-7 xl:space-x-9 items-center">
                    {navLinks.map((item) => (
                        ...
                        >
                            {item}
                            <span className={`absolute bottom-[-6px] left-0 w-0 h-0.5 bg-sky-500 transition-all duration-300 ease-in-out group-hover:w-full`}></span>
                        </a>
                    ))}
                </nav>
                <button
